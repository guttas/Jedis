...
class Hover(BaseTask):
    ...
    def __init__(self):
        ...
        self.target_z = 10.0  # target height (z position) to reach for successful Hover
        self.hover_zone = 1
    ...
    def update(self, timestamp, pose, angular_velocity, linear_acceleration):
        ...
        if abs(self.target_z - pose.position.z) < self.hover_zone:  # agent has crossed the target height
            reward = 20.0  # bonus reward
            #done = True
        if abs(self.target_z - pose.position.z) > self.target_z:
            reward -= 10.0
            done = True
        elif timestamp > self.max_duration:  # agent has run out of time
            if abs(self.target_z - pose.position.z) < self.hover_zone:
                reward = 20.0
            else:
