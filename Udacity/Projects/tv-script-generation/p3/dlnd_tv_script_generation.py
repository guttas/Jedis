...
def create_lookup_tables(text):
    ...
    vocab_to_int = {word : ii for ii, word in enumerate(vocabs)}
    int_to_vocab = {ii : word for ii, word in enumerate(vocabs)}
...
def get_batches(int_text, batch_size, seq_length):
    ...
    """
    # TODO: Implement Function
    batchenums = len(int_text)//(batch_size*seq_length)
    batches = np.zeros((batchenums, 2, batch_size, seq_length))
    #print(batches.shape)
    for batchnum in range(batchenums):
        inputs=np.zeros((batch_size, seq_length))
        targets=np.zeros_like(inputs)
        #print(inputs.shape)
        
        for n in range(batch_size):
            start = batchnum*seq_length + n * seq_length * batchenums
            end = start + seq_length
            
            inputs[n] = int_text[start:end]
            targets[n] = int_text[start+1:end+1]
            
        batches[batchnum] = np.array([inputs, targets])
